...
def get_word_score(word, n):
    ...
    returns: int >= 0
    """
    # Strips word & converts to lower case
    word = word.lower()
    word = word.strip()

    # Sums letter values to word value 
    word_value = 0 
    for letter in word:
        word_value += SCRABBLE_LETTER_VALUES[letter]

    # calculates seccond component of scoring
    word_len = len(word)
    seccond_comp_score = 7 * word_len - 3 * (n - word_len)

    # Checks seccond compoent or 1 is higher
    if seccond_comp_score < 1:
        seccond_comp_score = 1
    
    # Calculates total score
    score = word_value * seccond_comp_score
...
def is_valid_word(word, hand, word_list):
    ...
    # Wildcard used
        ...
        # Find wilcard index and creat list from word
        wc_index = word.find('*')
        chars = list(word)

        # Search through vowls replacing chars in list, & search word_list
        for vowle in VOWELS:
            chars[wc_index] = vowle
            if "".join(chars) in word_list:
                return True
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0:
        ...
        # Ask user for input
        word = input("Enter word, '!!' to indicate you are finished: ")
...
def substitute_hand(hand, letter):
    ...
    randomise_VorC = ("VOWLES", "CONSONANTS")

    if hand.get(letter) == None:
        print(f'"{letter}" not in hand')
        return hand
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    # Asks the user to input a total number of hands
    tot_hands = int(input("Enter total number of hands: "))
    current_hand = 0

    # Tracks total score
    total_score = 0

    # Counts substiotions only one allowed
